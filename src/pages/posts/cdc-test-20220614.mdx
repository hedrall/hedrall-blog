...
この、「どんなパラメータを送ったらどんなレスポンスをしてくれるのか？」をCDC Testingでは契約(Contract)と読んでいます。つまり、CDC Testing はシステム間のやり取り(Interaction)の詳細をスペック化(ドキュメント化)し、すのスペックに沿って出入力の正しさを確かめるテストです。
...
- 言語中立
  - パット見11言語に対応 (https://docs.pact.io/implementation_guides/cli)
- 認証・認可ヘの対応
  - => 専用のライフサイクルフックで再現可能
- 状態(DBのデータなど)
  - => 専用のライフサイクルフックで準備可能
