...
その一つとして、モデルの定義方法が挙げられます。DDDなどを実践するにあたって、
`class`構文や`Object Literal`型など、モデル定義の定義方法が複数あるために、どのパターンで実装するべきか悩むことがあります。
...
今回は以下の二軸でモデルの実装パターンを考えていきます。

1. 上から定義するか(継承)？下から定義するか？(Union型)
2. class(公称型) or Object Literal(構造的部分型)

`1.`に関して、「上から」「下から」というのは私の造語になりますが、
上からというのは一般的なオブジェクト志向言語のクラス定義方法で、はじめに親classを定義してから、継承(extend)を利用して子classを定義していく方法です。
一方で、下からというのはUnion Typeの性質を使って、子クラスの組み合わせを全体の集合(クラス)として定義する方法です。

`2.`に関して、一般的なオブジェクト志向言語で利用される`class`は公称型の性質を持ちますが、TypeScriptでよく利用される `Object Literal 型` は構造的部分型という異なった特徴を持ちます(詳細は後述)。
`1.`と`2.`の組み合わせとして、4通りの具体的なパターンを以下に示します。

今回の題材としてはよくあるTodoタスク管理アプリを想定しており、
...
この方法の問題は、親から見てどのような子孫が存在するのかがわからない点です。

これにより、主に2つのデメリットが考えられます。
1つはドキュメンテーションの低下です。
...
declare const task: Task;
switch (task) {
  // isTodoはTypeGuardとして定義されているものとする
  case isTodo(task /* <= Todo or Completed */):
    console.log('これから');
    break;
  // isCompletedはTypeGuardとして定義されているものとする
...
ちなみに、代入の際も公称型として扱う方法は存在します。

[参考](https://typescriptbook.jp/reference/object-oriented/class/class-nominality)
こちらの通り、privateなプロパティをもつclassは公称型の性質を持ちます。
...
- switch文では(`instanceof`で)型を絞り込むことができない
  - https://github.com/microsoft/TypeScript/issues/16035
...
より論理的に説明するされているすばらしい資料が公開されているので引用させていただきます。

[ビジネスロジックを「型」で表現するOOPのための関数型DDD](https://speakerdeck.com/yuitosato/functional-and-type-safe-ddd-for-oop)

この資料では、**代数的データ型**や**全域関数**と言った概念を用いて最適な方法が説明されており大変わかりやすいです。(脱帽です！)
...
そのため、共通する部分を括り出すことで、statusごとに何が共通していて、何が違うのかを明確に表現することができます。
...
必要に応じて、statusの定義を`Common`にジェネリクスとして追加することで強制していくことができます。
...
その場合、オリジナルの定義はなるべく一箇所にまとめた方が保守性が高くなるため、
...
今回紹介したTaskのように、親クラスに対して子クラスが有限なバリエーションを持つ関係形成については、結論として、下からObject Literalで定義する方法が最適であると考えました。
